...
# Define BEV mapping parameters (identical to render_bev.py)
...
for cam in cameras:
    ...
    valid_mask = (z_cam > 0) & (map_x >= 0) & (map_x < img_w - 1) & (map_y >= 0) & (map_y < img_h - 1)
...
for cam_a, cam_b in pairs:
    ...
    print(f"Overlap: {cam_a: <9} | {cam_b: <9} ({num_overlapping_pixels: >6} px) -> MAE: {mae:.2f}/255.0, RMSE: {rmse:.2f}")
