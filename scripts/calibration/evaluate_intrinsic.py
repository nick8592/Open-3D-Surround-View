    ...
    if ret:
        ...
        for i in range(7):
            ...
            for j in range(7):
                ...
                dist = abs((pt[0] - x[0])*vy[0] - (pt[1] - y[0])*vx[0]) / np.sqrt(vx[0]**2 + vy[0]**2)
                errors.append(dist)
        ...
        for i in range(7):
            cv2.line(vis_img, tuple(corners_refined[i, 0].astype(int)), tuple(corners_refined[i, 6].astype(int)), (0, 255, 0), 1)
            # Draw points
            for j in range(7):
                cv2.circle(vis_img, tuple(corners_refined[i, j].astype(int)), 3, (0, 0, 255), -1)
        for j in range(7):
            cv2.line(vis_img, tuple(corners_refined[0, j].astype(int)), tuple(corners_refined[6, j].astype(int)), (0, 255, 0), 1)
            
        plumbline_path = os.path.join(base_dir, "data/calibration/intrinsic/debug/test_plumbline.png")
        cv2.imwrite(plumbline_path, vis_img)
        print(f"Plumb-line debug image saved to: {plumbline_path}")
    else:
        print("Could not find checkerboard corners in original image for plumb-line evaluation.")
